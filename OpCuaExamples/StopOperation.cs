...
namespace OPCUAExamples
{
    ...
    {
        /// <summary>
        /// Configures and calls the "Stop" OPC UA method.
        ...
        public void CallStopOperation()
        {
            ...
            // Find the reference description for the "Stop" method from the pre-browsed collection.
            // Assumes ConnectionManager._methodCollectionPlayControl holds method nodes for "PlayControl".
            // ViCellBlu.BrowseNames.Stop provides the string literal "Stop".
            var methodStop = ConnectionManager._methodCollectionPlayControl.First(n => n.DisplayName.ToString().Equals(ViCellBlu.BrowseNames.Stop)); //Stop
                                                                                                                                                            // Convert the method's ExpandedNodeId (from browsing) to a Session-specific NodeId.
            var methodNodeStop = ExpandedNodeId.ToNodeId(methodStop.NodeId, ConnectionManager._opcSession.NamespaceUris);

            Console.WriteLine($"Preparing to call method: {methodStop.DisplayName.Text}");

            // Create a RequestHeader (often default for simple calls).
            var reqHeaderStop = new RequestHeader();
            // Create the CallMethodRequest object.
            var cmRequestStop = new CallMethodRequest
            {
                ...
                MethodId = methodNodeStop,
                // InputArguments: A collection of input arguments for the method.
                // The Stop method expects one input argument: the SampleConfig object.
            ...
            var cmReqCollectioncmRequestStop = new CallMethodRequestCollection { cmRequestStop };

            // Show what is being sent to the server.
            Console.WriteLine("Sending:");
            Console.WriteLine($"  Command: {methodStop.DisplayName.Text}");
            ...
            ConnectionManager._opcSession.Call(
                reqHeaderStop,                  // Request header
                cmReqCollectioncmRequestStop,   // Collection of methods to call
                out var resultCollectionStop, // Output: Collection of results from the calls
                out var diagResultsStop         // Output: Diagnostic information
                );

            // --- Process Results ---
            // Get the result for the first (and only) method call in the collection.
            var result = resultCollectionStop[0];
            ...
            if ((resultCollectionStop.Count > 0) && (resultCollectionStop[0].OutputArguments.Count > 0))
            {
                ...
                callResult = DecodeRaw(
                    resultCollectionStop[0].OutputArguments[0].Value, // The raw output value
            ...
            else
            {
                ...
                // but returned no output arguments (which shouldn't happen for Stop if successful).
                ...
                if (diagResultsStop != null && diagResultsStop.Count > 0)
                {
                    Console.WriteLine($"Diagnostic result: {diagResultsStop.ToString()}");
                }
            }
