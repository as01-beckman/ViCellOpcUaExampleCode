...
{
    /// <summary>
    /// Class to demonstrate calling the "StartSample" OPC UA method on a Vi-Cell BLU server.
    /// This example is specifically configured for starting an A-cup sample.
    ...
    /// </summary>
    public class StartSampleOperation
    {
        /// <summary>
        ...
        /// </summary>
        public void CallStartSample()
        {
            // --- Sample Configuration ---
            // Define the available default cell types supported by the Vi-Cell BLU.
            // These strings must match the server's expected values.
            ...
            // Create a new SampleConfig object with details for the sample run.
            // It's crucial that for a new sample, the UUID is not pre-assigned by the client;
            // the server will assign a unique ID upon receiving the config.
            var sampleConfig = new ViCellBlu.SampleConfig
            {
                SampleName = "A-cupSampleFromOPCUAExample", 
                SamplePosition = new ViCellBlu.SamplePosition { Column = 1, Row = "Y" },
                Tag = "ACupSampleTag", 
                Dilution = 1, 
                CellType = new ViCellBlu.CellType { CellTypeName = "Mammalian" }, 
                QualityControl = new ViCellBlu.QualityControl { QualityControlName = string.Empty },
                SaveEveryNthImage = 1,
                WashType = 0
            };

            // Print the configured sampleConfig values to the console for verification.
            Console.WriteLine("SampleConfig values:");
            Console.WriteLine($"  SampleName: {sampleConfig.SampleName}");
            Console.WriteLine($"  SamplePosition: Column={sampleConfig.SamplePosition.Column}, Row={sampleConfig.SamplePosition.Row}");
            Console.WriteLine($"  Tag: {sampleConfig.Tag}");
            Console.WriteLine($"  Dilution: {sampleConfig.Dilution}");
            Console.WriteLine($"  CellType: {sampleConfig.CellType?.CellTypeName ?? "Not Set"}"); // Use null check for safety
            Console.WriteLine($"  QualityControl: {sampleConfig.QualityControl?.QualityControlName ?? "Not Set"}"); // Use null check for safety
            Console.WriteLine($"  SaveEveryNthImage: {sampleConfig.SaveEveryNthImage}");
            Console.WriteLine($"  WashType: {sampleConfig.WashType}");
            ...
            var methodStartSample = ConnectionManager._methodCollectionPlayControl.First(n => n.DisplayName.ToString().Equals(ViCellBlu.BrowseNames.StartSample));
